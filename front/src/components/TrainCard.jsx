import { Train, ExternalLink, Clock, Zap } from "lucide-react";
import { cn } from "@/lib/utils";

function fmtHHmm(s) { return (s || "").slice(0, 5); }
...
export default function TrainCard({ trip }) {
  const isInoui = trip.train_type === "TGV_INOUI";
  ...
  return (
    ...
    >
      {/* Left ribbon */}
      <div className={cn("sm:w-2 h-2 sm:h-auto", isInoui ? "bg-[#E11D48]" : "bg-[#0284C7]")} />
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <span
              className={cn(
                ...
                isInoui ? "bg-[#E11D48] text-white" : "bg-[#0284C7] text-white"
              )}
              data-testid="train-type-badge"
            >
              {isInoui ? "TGV INOUI" : "INTERCITÉS"}
            </span>
            {imminent && (
